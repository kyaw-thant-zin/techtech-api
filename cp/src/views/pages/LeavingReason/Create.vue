...
watchEffect(() => {
    // set area rows
    if(lReasonStore._loading) {
        $q.loading.show()
    } else {
        $q.loading.hide()
    }

}, [lReasonStore._loading])

// create new area
const onSubmit = async () => {
    await lReasonStore.handleStore(formData.value)
    if(lReasonStore._success) {
        ...
        resetForm()
        lReasonStore.router.replace({ name: 'cp.leaving-reason' })
    }
